...
- Refreshed GA readiness and release-owner approval evidence to exact-head `b970c07` hosted CI and
  generated-output cleanup fingerprints.
...
- Hardened hosted CI release evidence so release-gate run/PR checks and pre-step blocker
  verification target an explicit repository instead of ambient `gh` context.
...
- Hardened the GA release gate so stale generated archive, checksum, or Homebrew formula outputs
  fail a full local-proof preflight before local CI/package/Homebrew validation can run.
...
- Hardened release evidence collection so non-default hosted CI event selection requires
  `ALLOW_EXPECTED_EVENT_OVERRIDE=1` before GitHub run discovery or inspection.
