# Native strict successor Stage C2B2a — enforcement closure V4
...
This is an append-only successor to V3, which failed its coordinator freeze audit before any
external design review:

```text
SHA-256 46b9276d626324508841d9163ceb8c3c8d3a659c72fc305981a7be56169f9aae
lines   378
bytes   24449
path    evals/native_memory_pilot_v1/NATIVE_STRICT_SUCCESSOR_STAGE_C2B2A_EXTERNAL_SOURCE_AUTHORIZATION_BOUNDARY_TWO_MANIFEST_ENFORCEMENT_SUCCESSOR_CLOSURE_V3_FROZEN_2026-09-10.md
state   rejected; its prospective review stayed absent; no authority transfers
```

V3 attempted to identify rejected V2 in its disposition, but paired V2's hash, line count, and byte
count with V3's own path. That historical tuple was false, so V3 was rejected locally before review.
No review verdict exists or transfers. V4 corrects that disposition and otherwise carries forward
V3's substantive stable-parent and enforcement repairs.

The rejected V2 epoch remains exactly:

```text
SHA-256 a5e9ed0d7ab33cf9ef2714ef62d8e638a851f2b7ecbf96b02a7cef2af0a8ef9d
lines   372
bytes   23523
path    evals/native_memory_pilot_v1/NATIVE_STRICT_SUCCESSOR_STAGE_C2B2A_EXTERNAL_SOURCE_AUTHORIZATION_BOUNDARY_TWO_MANIFEST_ENFORCEMENT_SUCCESSOR_CLOSURE_V2_FROZEN_2026-09-10.md
state   rejected; its prospective review stayed absent; no authority transfers
```

V2 froze audit-parent nlink `128`, but its own create-new file transition changed the observed APFS
directory nlink to `129`. The supposedly stable precondition therefore invalidated itself. No
review verdict exists or transfers. V4 excludes directory nlink from stable parent identity and
requires exact entry-set transition checks instead.

The earlier rejected bbdb epoch remains exact SHA-256
`bbdb5af7f55f5ee6c504f3ccbf3cf02c0711e5404fc33bc51366daa72700aaec`, 515 lines, 32801 bytes.
Its blind reviews were `FAIL 0/5/2` (Codex normative), `PASS 0/0/3` (Codex guard), and
`PASS 0/0/4` (isolated Claude); aggregate FAIL. Rejected predecessors `5fac7622...`,
`bed96069...`, `74c883fe...`, `f1d7ddc8...`, `7d0ae76f...`, and unavailable historical
`1cd2c842...` remain immutable raw evidence and confer no authority. The inherited frozen
boundary/reconciliation/carrier records and unchanged candidate source hashes remain those recorded
by V2.

Recursive candidate self-checks remain rejection-only. Success never grants permission. This V4
document supersedes V3, V2, and bbdb only for historical-disposition accuracy,
audit-parent identity, external-review serialization, authority-DAG serialization,
...
```text
evals/native_memory_pilot_v1/NATIVE_STRICT_SUCCESSOR_STAGE_C2B2A_EXTERNAL_SOURCE_AUTHORIZATION_BOUNDARY_TWO_MANIFEST_ENFORCEMENT_SUCCESSOR_CLOSURE_V4_FROZEN_2026-09-10.md
```

Its prospective consolidated review path is exactly:

```text
evals/native_memory_pilot_v1/NATIVE_STRICT_SUCCESSOR_STAGE_C2B2A_EXTERNAL_SOURCE_AUTHORIZATION_BOUNDARY_TWO_MANIFEST_ENFORCEMENT_SUCCESSOR_CLOSURE_V4_REVIEW_2026-09-10.md
```
...
`700150912`, UID `502`, GID `20`, and mode `0755`. Directory nlink, size, mtime, and ctime are
transition-sensitive and deliberately excluded from stable parent identity because a permitted
create-new leaf can change them. Before and after amendment/review creation, coordinator records
those values as evidence and instead requires the exact direct-entry set transition: existing
entries byte/metadata identical, exactly the authorized new basename added, no other delta. Any
stable parent field change or unlisted entry delta is terminal and requires a newly named successor.
The exact review basename is the prospective basename above;
...
- every field is one literal ASCII field name, equals sign, canonical value, then LF;
- a text value is unsigned-decimal raw-byte length, colon, then lowercase hex of exactly its UTF-8
  bytes; an integer/count value is minimal unsigned decimal; decimal has no leading zero except `0`;
...
epoch, phase, coordinator commitment, capability, target path, target device, target inode, target UID, target GID, target mode, target nlink, target
size, target LF count, target byte count, target final byte, target SHA-256. Names are literal ASCII; numeric values are
...
pre/post-spawn times. On Darwin the birth tuple is PID plus the exact seconds/microseconds start fields
returned by `proc_pidinfo(PROC_PIDTBSDINFO)`; another host requires an equivalently frozen reviewed
API tuple before Gate0 and cannot silently substitute PID alone. FD3 is close-on-exec for the scanner and never inherited by it.
...
inode/UID/GID/mode/nlink/size/content SHA/byte count/LF count/final byte, where final byte is two
lowercase hex digits for nonempty files and the literal `empty` for zero-byte files. Directory entries use
