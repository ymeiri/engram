# T230 Approval Packet: T217/T221/T223/T225/T227/T229 Runtime Refresh
...
the T225, T227, and T229 focused source fixtures.
...
force-kill, deletion, old-binary reinstall, or user-owned file change has been run for T230.

T230 supersedes T228. T228 was correct when authored, but T229 intentionally changed
binary-relevant `engram-mcp/src/tools.rs` after T228 to add MCP tool-level coverage for
`telemetry(action="record_trace")` runtime env fallback. The old T228 first checks must now stop if
executed. Any future runtime refresh should use this T230 packet instead.

T230 intentionally allows only:
...
Can Engram safely refresh the installed MCP runtime to include T217, T221, T223, T225, T227, and
T229, then prove live that:
...
6. the T225 combined project-name-only plus `limit=1` path works in live runtime; and
7. the T227 startup-style project-name-only plus `tags=["current-plan"]` path works in live
   runtime.
...
| Type | Hypothesis |
| --- | --- |
| Preferred | If binary-relevant source remains at baseline `d953d16`, installing `/Users/yuval.meiri/.local/bin/engram`, cleanly restarting the daemon once with a unique temporary `ENGRAM_EXTERNAL_SESSION_ID`, and exercising the T217/T221/T223/T225/T227/T229 live MCP paths will prove all source fixes in installed runtime. A final restart without the temporary env will prove the label is not sticky. |
...
| Simpler alternative | Keep T228 as the active packet and ask for approval anyway. Rejected because T228's binary-relevant invariant is now stale after T229. |
...
- T228 approval packet is committed as `681081f` but is now stale for execution because T229
  changed binary-relevant MCP source after it.
...
- Fresh read-only diff checks from T229 baseline
  `d953d16d857d0457d3d1d951eccbc630d33a28b1` to current HEAD show no binary-relevant committed,
...
- T227 validation passed:
  ...
  - `cargo check -p engram-cli`
  - `git diff --check`
- T229 validation passed:
  - `cargo test -p engram-mcp tools::tests::mcp_telemetry_record_trace_uses_runtime_env_when_request_is_absent -- --exact`
  - `cargo test -p engram-mcp tools::tests` with 6 passing tests
  - `cargo test -p engram-tests --test telemetry_tests` with 24 passing tests
  - `cargo fmt --all --check`
  - `cargo check -p engram-cli`
  - `git diff --check`
...
T230 allows committed docs-only approval/report drift from source baseline
`d953d16d857d0457d3d1d951eccbc630d33a28b1` only if fresh execution-start checks prove no
...
```text
git diff --name-status d953d16d857d0457d3d1d951eccbc630d33a28b1..HEAD -- Cargo.toml Cargo.lock engram-core engram-store engram-embed engram-index engram-mcp engram-cli engram-tests scripts .cargo
...
env ENGRAM_EXTERNAL_SESSION_ID=t230-runtime-env-<stamp> /Users/yuval.meiri/.local/bin/engram daemon start
```
...
All validation output should be recorded in a T230 execution report. Do not write active memory
...
- an omitted-label telemetry trace no longer uses the `t230-runtime-env-<stamp>` label.

## Exact Approval Phrase

Approve T230: execute the T217/T221/T223/T225/T227/T229 runtime refresh from
...
temporarily start the daemon with `ENGRAM_EXTERNAL_SESSION_ID=t230-runtime-env-<stamp>` for
...
`memory(changes_since)`, read-only T227 `memory(list)` project_name-only current-plan tag
filtering, read-only T221/T225 `memory(list)` project_name-only scope filtering with `limit=1`,
read-only T223 explicit scoped post-filter limit behavior, then restart the daemon without the
temporary env and prove the label is not sticky. Do not execute stale T219, T222, T224, T226, or
T228, edit installed hooks/settings/adapters or user-owned files, run harness install, use
adopt_user_owned, run native Claude, run Claude Bridge writes, run M6/migration/quarantine, mutate
lifecycle state, run lint apply_safe, change public MCP params or response shape, change ranking/
`orient` payload, schema/storage/index/document-index behavior, PATH/profile/auth/service
configuration outside the temporary process env used by this packet, rollback, force-kill,
