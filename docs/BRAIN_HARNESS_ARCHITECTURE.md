...
- The harness claim remains bounded. Lifecycle compliance is still a soft contract; Claude Code
  ...
  source-level MCP `ENGRAM_EXTERNAL_SESSION_ID` fallback support now exist; T219 has prepared the
  exact runtime-refresh approval gate, but the installed runtime has not been refreshed for the MCP
  fallback; stale active handoffs remain until a future non-dry-run handoff update or explicit
  lifecycle cleanup.
