# T224 Approval Packet: T217/T221/T223 Runtime Refresh
...
telemetry/read-only live validation for the committed T217, T221, and T223 MCP source fixes.
...
force-kill, deletion, old-binary reinstall, or user-owned file change has been run for T224.

T224 supersedes T222. T222 was correct when authored, but T223 introduced intentional
binary-relevant source drift in `engram-mcp/src/tools.rs` and
`engram-tests/tests/memory_tests.rs`. The old T222 first checks must now stop if executed. Any
future runtime refresh should use this T224 packet instead.

T224 intentionally allows only:
...
- read-only live MCP memory-list validation needed to prove T221 project-scope inference and T223
  scoped post-filter limit behavior.
...
Can Engram safely refresh the installed MCP runtime to include T217, T221, and T223, then prove live
that:
...
| Type | Hypothesis |
| --- | --- |
| Preferred | If binary-relevant source remains at baseline `19707e6`, installing `/Users/yuval.meiri/.local/bin/engram`, cleanly restarting the daemon once with a unique temporary `ENGRAM_EXTERNAL_SESSION_ID`, and exercising the T217/T221/T223 live MCP paths will prove all three source fixes in installed runtime. A final restart without the temporary env will prove the label is not sticky. |
| Null | Runtime refresh or validation does not demonstrate one or more fixes: the wrong binary is serving MCP, daemon env did not propagate, telemetry observation is ambiguous, live memory data cannot demonstrate the scoped-limit invariant, or source tests missed a live-path issue. |
| Simpler alternative | Keep T222 as the active packet and ask for approval anyway. Rejected because T222's binary-relevant invariant is now stale after T223. |
...
- T222 approval packet is committed as `439d532` but is now stale for execution because T223
  changed binary-relevant source after it.
...
- Fresh read-only diff checks show binary-relevant drift from T222's baseline to current HEAD:
  - `M engram-mcp/src/tools.rs`
  - `M engram-tests/tests/memory_tests.rs`
- Fresh read-only diff checks from T223 baseline `19707e6` to current HEAD show no
...
- T223 validation passed:
  ...
  - focused adjacent scope/tag regressions
  - `cargo test -p engram-tests --test memory_tests` with 32 passing tests
  - `cargo fmt --all --check`
  - `cargo check -p engram-cli`
  - `git diff --check`
...
T224 allows committed docs-only approval/report drift from source baseline
`19707e60b9126b2fcdfabbe5fe9c0562a44c7f03` only if fresh execution-start checks prove no
binary-relevant committed, staged, or unstaged drift.
...
```text
git diff --name-status 19707e60b9126b2fcdfabbe5fe9c0562a44c7f03..HEAD -- Cargo.toml Cargo.lock engram-core engram-store engram-embed engram-index engram-mcp engram-cli engram-tests scripts .cargo
...
env ENGRAM_EXTERNAL_SESSION_ID=t224-runtime-env-<stamp> /Users/yuval.meiri/.local/bin/engram daemon start
```
...
T221/T223 read-only memory-list validation:
...
- run `memory(action="list", status_filter="active", project_name="engram", limit=1)`;
  ...
  inference plus limit preservation on the live path;
...
All validation output should be recorded in a T224 execution report. Do not write active memory
...
- an omitted-label telemetry trace no longer uses the `t224-runtime-env-<stamp>` label.

## Exact Approval Phrase

Approve T224: execute the T217/T221/T223 runtime refresh from
`docs/BRAIN_HARNESS_T224_T217_T221_T223_RUNTIME_REFRESH_APPROVAL_PACKET_2026-06-04.md`. Install
the current `engram-cli` binary to `/Users/yuval.meiri/.local`, restart the Engram daemon,
temporarily start the daemon with `ENGRAM_EXTERNAL_SESSION_ID=t224-runtime-env-<stamp>` for
...
read-only T221 `memory(list)` project_name-only scope filtering, read-only T223 scoped
post-filter limit behavior, then restart the daemon without the temporary env and prove the label
is not sticky. Do not execute stale T219 or T222, edit installed hooks/settings/adapters or
user-owned files, run harness install, use adopt_user_owned, run native Claude, run Claude Bridge
writes, run M6/migration/quarantine, mutate lifecycle state, run lint apply_safe, change public MCP
params or response shape, change ranking/`orient` payload, schema/storage/index/document-index
behavior, PATH/profile/auth/service configuration outside the temporary process env used by this
packet, rollback, force-kill, deletion, or old-binary reinstall.
