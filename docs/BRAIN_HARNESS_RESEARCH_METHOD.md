...
- T100 applies the T91/T94/T96/T98 continuity rule after T99: compare `orient`, direct search,
  `changes_since`, `handoff(get)`, docs, git state, and source before trusting resume state. If
  only the rolling handoff is stale, one handoff refresh is acceptable continuity maintenance, but
  it must not be treated as archive approval, migration approval, document-index approval, ranking
  evidence, `orient` expansion, public MCP change, schema/storage/index change, or harness work.
...
- T102 applies the T91/T94/T96/T98/T100 continuity rule after T101: compare `orient`, direct
  search, `changes_since`, `handoff(get)`, docs, git state, and source before trusting resume state.
  If only the rolling handoff is stale, one handoff refresh is acceptable continuity maintenance, but
  it must not be treated as archive approval, migration approval, document-index approval, ranking
  evidence, `orient` expansion, public MCP change, schema/storage/index change, or harness work.
- T103 applies the T88/T95/T97/T99/T101 lifecycle-packet pattern to the T100 handoff newly
  superseded by T102. Preparing an exact archive approval packet is acceptable evidence-quality
  work, but it must freeze one target ID, avoid `lint(action="apply_safe")` and broad handoff
  cleanup, and require exact ID-scoped approval before any lifecycle write.
- T104 applies the T91/T94/T96/T98/T100/T102 continuity rule after T103 and an external Claude
  Bridge side-effect: compare `orient`, direct search, `changes_since`, `handoff(get)`, docs, git
  state, and source before trusting resume state. If only the rolling handoff is stale, one handoff
...
- T107 applies the current-plan ranking rule to direct `search` only, not `orient`: if live evidence
  ...
  expansion.
- T108 applies the stale-current-plan lifecycle rule without mutating lifecycle state: if fresh
  evidence shows `019e5e0a-86b4-73e3-aa9b-ca350e83e915` remains active, repository-scoped,
  current-plan tagged, repeatedly marked stale, and ranked below the active T107 project current
  plan, a docs-only evidence snapshot may freeze that exact target. Because lint reports
  `safe_action=none`, the artifact must be framed as default-deny evidence, not proxy approval; it
