# T226 Approval Packet: T217/T221/T223/T225 Runtime Refresh
...
the T225 combined source fixture.
...
force-kill, deletion, old-binary reinstall, or user-owned file change has been run for T226.

T226 supersedes T224. T224 was correct when authored, but T225 intentionally changed
binary-relevant `engram-tests/tests/memory_tests.rs` after T224 to add combined source coverage for
the exact live validation path. The old T224 first checks must now stop if executed. Any future
runtime refresh should use this T226 packet instead.

T226 intentionally allows only:
...
- read-only live MCP memory-list validation needed to prove T221 project-scope inference, T223
  scoped post-filter limit behavior, and the combined T225 path.
...
Can Engram safely refresh the installed MCP runtime to include T217, T221, T223, and T225, then
prove live that:
...
4. scoped `memory(action="list", ..., limit=1)` applies the requested limit after scope filtering,
   not only after tag filtering; and
5. the combined project-name-only plus `limit=1` path covered by T225 works in live runtime.

## Hypotheses

| Type | Hypothesis |
| --- | --- |
| Preferred | If binary-relevant source remains at baseline `ff2d6fd`, installing `/Users/yuval.meiri/.local/bin/engram`, cleanly restarting the daemon once with a unique temporary `ENGRAM_EXTERNAL_SESSION_ID`, and exercising the T217/T221/T223/T225 live MCP paths will prove all source fixes in installed runtime. A final restart without the temporary env will prove the label is not sticky. |
...
| Simpler alternative | Keep T224 as the active packet and ask for approval anyway. Rejected because T224's binary-relevant invariant is now stale after T225. |
...
- T224 approval packet is committed as `0c100a4` but is now stale for execution because T225
...
- Fresh read-only diff checks from T225 baseline `ff2d6fd` to current HEAD show no binary-relevant
...
- T225 validation passed:
  ...
  - `cargo test -p engram-tests --test memory_tests` with 33 passing tests
  - `cargo fmt --all --check`
  - `cargo check -p engram-cli`
  - `git diff --check`
- Current read-only live pre-state:
  ...
  - read-only live `memory(action="list", project_name="engram", status_filter="active",
    limit=3)` still returned one `dd-source` item, proving the installed runtime has not picked up
    the T221/T223/T225 memory-list fixes yet.

## Binary-Relevant Invariant

T226 allows committed docs-only approval/report drift from source baseline
`ff2d6fd5199279eb96b9d2e2e044cece4cd23607` only if fresh execution-start checks prove no
...
```text
git diff --name-status ff2d6fd5199279eb96b9d2e2e044cece4cd23607..HEAD -- Cargo.toml Cargo.lock engram-core engram-store engram-embed engram-index engram-mcp engram-cli engram-tests scripts .cargo
...
env ENGRAM_EXTERNAL_SESSION_ID=t226-runtime-env-<stamp> /Users/yuval.meiri/.local/bin/engram daemon start
```
...
T221/T223/T225 read-only memory-list validation:
...
All validation output should be recorded in a T226 execution report. Do not write active memory
...
- an omitted-label telemetry trace no longer uses the `t226-runtime-env-<stamp>` label.

## Exact Approval Phrase

Approve T226: execute the T217/T221/T223/T225 runtime refresh from
...
temporarily start the daemon with `ENGRAM_EXTERNAL_SESSION_ID=t226-runtime-env-<stamp>` for
...
read-only T221/T225 `memory(list)` project_name-only scope filtering with `limit=1`, read-only T223
explicit scoped post-filter limit behavior, then restart the daemon without the temporary env and
prove the label is not sticky. Do not execute stale T219, T222, or T224, edit installed
