...
`eefe20416db209d646334e7759b4130c0ee730c4`
...
- Current release-gate behavior CI: main push run `27511071899` for
  `eefe20416db209d646334e7759b4130c0ee730c4` completed successfully on
  2026-06-14 for Check, Test, Clippy, Docs, and Format. The post-CI GA gate
  assertions now include the quick `evidence_incomplete` state, the default
  `generated_outputs_cleanup_required` full-gate stop, the read-only
  `generated_output_cleanup_fingerprints_verified` cleanup verifier state, the exact
  three-path cleanup allowlist for the archive, checksum, and Homebrew formula, and cargo
  regressions that reject cleanup manifests if they claim cleanup already happened or if any
  stale generated-output fingerprint changes. Commit `eefe204` also added a regression proving the
  verifier emits a stable `cleanup_fingerprint_sha256` for the normalized stale-output set even
  when raw gate-manifest metadata changes.
...
- Cleanup-verifier regression CI: main push run `27511071899` for
  `eefe20416db209d646334e7759b4130c0ee730c4` completed successfully on
  2026-06-14 for Check, Test, Clippy, Docs, and Format. Commit `cd94e1a` added
  temporary-repository release-gate tests that verify a matching generated-output cleanup manifest
  succeeds, a manifest with `actions_performed.generated_output_cleanup=true` is rejected, and
  any changed stale output fingerprint is rejected. Commit `eefe204` added the
  manifest-metadata-stability regression for `cleanup_fingerprint_sha256`. A fresh post-fetch
  branch check had
...
- Current cleanup-approval quick GA gate:
  `scripts/release-gate-report.sh --target ga --hosted-run 27511071899 --quick --json`
  passed for exact head `eefe204`, with hosted CI
  passing, release target `v0.2.0` still available, generated outputs reported as
  read-only cleanup evidence with regular-file size and SHA-256 fingerprints, and
  `release_actions_performed=false`.
...
- Current post-CI full-gate rehearsal against run `27511071899` keeps
...
- The latest exact-head default GA gate on this host passes disk preflight with
  ...
  CI/package smoke on head `eefe204` with
  ...
  The matching read-only cleanup verifier reports
  `cleanup_fingerprint_sha256=953a22106e39b416e351b3fb86354f59215c8c6a4b05d88ce4711b7be8acc6f4`
  with equal expected/current output-set hashes and no release actions.
