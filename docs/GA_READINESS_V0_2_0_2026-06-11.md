...
`cd94e1a68f59a9a7bc373f2a3b1a7d5639444fa2`
...
- Current release-gate behavior CI: main push run `27509155255` for
  `cd94e1a68f59a9a7bc373f2a3b1a7d5639444fa2` completed successfully on
  ...
  stale generated-output fingerprint changes. The verifier also emits a stable
  `cleanup_fingerprint_sha256` for the normalized stale-output set so release-owner approval can
  distinguish the file cleanup fingerprint from the raw gate-manifest provenance hash.
...
- Cleanup-verifier regression CI: main push run `27509155255` for
  `cd94e1a68f59a9a7bc373f2a3b1a7d5639444fa2` completed successfully on
  ...
  any changed stale output fingerprint is rejected. A fresh post-fetch branch check had
...
- Current cleanup-approval quick GA gate:
  `scripts/release-gate-report.sh --target ga --hosted-run 27509155255 --quick --json`
  passed for exact head `cd94e1a`, with hosted CI
  passing, release target `v0.2.0` still available, generated outputs reported as
  read-only cleanup evidence with regular-file size and SHA-256 fingerprints, and
  `release_actions_performed=false`.
...
- Current post-CI full-gate rehearsal against run `27509155255` keeps
...
- The latest exact-head default GA gate on this host passes disk preflight with
  ...
  CI/package smoke on head `cd94e1a` with
