...
T35 matrix note: after the fixed-case audit and subsequent startup-trace feedback scoring,
...
`bad_memory_used_count=0`, `task_failure_count=1`, and `confidence_gate.passed=true`. The
`prepare_handoff` fixed-case failure remains the controlling evidence; do not treat the aggregate
gate pass as product completion or migration approval.
