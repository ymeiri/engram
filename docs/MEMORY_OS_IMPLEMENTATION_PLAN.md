...
- [x] T18 read-only evidence gate re-audit: current `real_session_eval(project=engram, limit=50)`
      no longer passes the confidence gate because the current sample has feedback across only two
      intents. Read-only `lint(action=apply_safe, write=false)` reported `applied_safe_actions=0`.
      The stale repository-scoped current-plan memory
      `019e5e0a-86b4-73e3-aa9b-ca350e83e915` now has 42 stale-feedback hits and still appears
      near the latest current plan in review-memory search. Archival/scope correction is a memory
      lifecycle write and needs explicit approval; document-index normalization also needs approval
