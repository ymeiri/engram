...
| Risky | Telemetry is agent-assessed, sampled, and window-sensitive; fresh telemetry windows still fail confidence gates. Harness lifecycle compliance is soft; stale or docs-only packets such as T255, T269, T282, T312, T313, T334, and T335 can be mistaken for executed cleanup/validation; quarantined T278 entity-scope M6 candidates are not active MemoryItems; untracked root `AGENTS.md` remains user-owned and out of commits. | Keep scope wording exact and keep scoring material traces. |
