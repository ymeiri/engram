...
serializes the CI Test command with `cargo test --all-targets --jobs 1`; local `git diff --check`,
`cargo fmt --all --check`, `cargo clippy --all-targets -- -D warnings`, and
`cargo test --all-targets --jobs 1` pass.
...
| PR CI follow-up | T285 fixes the first failing PR #2 CI run: Clippy's timestamp sort warnings and Test's concurrent linker bus-error failure mode. Local `git diff --check`, `cargo fmt --all --check`, `cargo clippy --all-targets -- -D warnings`, and `cargo test --all-targets --jobs 1` pass. | Commit, push, and watch the fresh GitHub Actions run. Treat the PR as not remotely green until the pushed run passes. |
