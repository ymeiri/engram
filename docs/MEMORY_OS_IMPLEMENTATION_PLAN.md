...
- [x] T35 pre-registered evidence-quality audit: fixed read-only `review_memory`,
      ...
      confidence gate passed numerically afterward, but this is not product proof and does not
      authorize `orient`, ranking, lifecycle, migration, hook, adapter, schema/storage, public MCP,
      or telemetry formula changes.
