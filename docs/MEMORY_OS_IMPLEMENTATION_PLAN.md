...
and `cargo test --all-targets --jobs 1` passed before the Rust 1.96-only follow-up.
...
| PR CI follow-up | T285 fixes the first failing PR #2 CI run: Clippy's timestamp sort warnings and Test's concurrent linker bus-error failure mode. The first pushed fix surfaced one additional Rust 1.96-only `collapsible_match` warning in `engram-index/src/harness.rs`, also fixed in T285. Local `git diff --check`, `cargo fmt --all --check`, `cargo clippy --all-targets -- -D warnings`, and `cargo test --all-targets --jobs 1` passed before the Rust 1.96-only follow-up. | Commit, push, and watch the fresh GitHub Actions run. Treat the PR as not remotely green until the pushed run passes. |
