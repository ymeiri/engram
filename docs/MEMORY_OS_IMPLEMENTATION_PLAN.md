...
Clippy, and Test on head `688f8fe75f03a62e1712185258010edb22ae4574`. The beta readiness bar is
green CI on the exact head, validated Codex/local Brain Loop behavior, canonical vault
init/compile, lean `orient` with current-plan/used-memory/obligation summary, M6
inventory/export/inspection/status paths, supported-path doctors, explicit beta limitations, and
preserved approval boundaries. Native Claude prompt-bearing proof, effective-hook visibility, live
Claude host-label proof, direct legacy deprecation/deletion, exhaustive lifecycle cleanup after
T294, broad `lint apply_safe`, and full multi-host harness parity are deferred known limitations,
not initial beta blockers.

Current T295 matrix snapshot:
