...
pub struct MemoryVaultExport {
    ...
    /// Obsolete Engram-generated files removed during recompilation.
...
pub(crate) fn write_memory_vault(
    ...
    let expected_paths = expected_generated_paths(
        &sorted_items,
        &sorted_commits,
        &sorted_repositories,
        &entity_paths,
    ...
    remove_obsolete_generated_files(root, &expected_paths, &mut export)?;

    export.files_written.sort();
    export.files_skipped.sort();
    export.files_removed.sort();
    Ok(export)
}

fn expected_generated_paths(
    items: &[MemoryItem],
    commits: &[KnowledgeCommit],
    repositories: &[RepositoryVaultSnapshot],
    ...
    let mut paths = BTreeSet::from([
        PathBuf::from("99_System/Vault-Index.md"),
        PathBuf::from("memory/index.md"),
        PathBuf::from("entities/index.md"),
        PathBuf::from("repositories/index.md"),
    ]);
    paths.extend(items.iter().map(item_relative_path));
    paths.extend(commits.iter().map(commit_relative_path));
    paths.extend(
        repositories
            .iter()
            .map(|snapshot| repository_relative_path(&snapshot.repository)),
    );
    paths.extend(entity_paths.values().cloned());
    paths.extend(project_paths.values().cloned());
    paths
}
...
fn remove_obsolete_generated_files(
    ...
    fn visit(
        ...
        for entry in fs::read_dir(path)? {
            ...
            if file_type.is_dir() {
                visit(root, &entry_path, expected_paths, export)?;
                continue;
            }
            if !file_type.is_file() {
                continue;
            }
            let relative_path = entry_path.strip_prefix(root).map_err(|_| {
                IndexError::InvalidState(format!(
                    "vault path escaped export root: {}",
                    entry_path.display()
                ))
            })?;
            ...
            if String::from_utf8_lossy(&contents).contains(GENERATED_MARKER) {
                ...
                export.files_removed.push(path_to_markdown(relative_path));
            }
        }
        Ok(())
    }

    visit(root, root, expected_paths, export)
}
    ...
    if let Some(procedure) = &item.procedure {
        ...
        if !procedure.prerequisites.is_empty() {
            ...
            for prerequisite in &procedure.prerequisites {
                output.push_str(&format!(
                    "  - `{}` = `{}`\n",
                    prerequisite.key, prerequisite.expected
                ));
            }
        }
        ...
        if !procedure.failure_signatures.is_empty() {
            output.push_str("- Known failure signatures:\n");
            for signature in &procedure.failure_signatures {
                output.push_str(&format!("  - `{signature}`\n"));
            }
        }
        output.push_str(&format!(
            "- Verification: `{}` exits {} and output contains `{}`\n",
            procedure.verification.command,
            procedure.verification.expected_exit_code,
            procedure.verification.expected_output_contains
        ));
        if let Some(path) = &procedure.verification.evidence_path {
            output.push_str(&format!("- Verification receipt: `{path}`\n"));
        }
        if let Some(hash) = &procedure.verification.evidence_sha256 {
            output.push_str(&format!("- Verification receipt SHA-256: `{hash}`\n"));
        }
        if let Some(verified_at) = procedure.verification.verified_at {
            output.push_str(&format!("- Verified at: {}\n", format_time(verified_at)));
        }
...
mod tests {
    ...
    #[test]
    fn write_memory_vault_removes_only_obsolete_generated_pages() {
        let dir = tempdir().unwrap();
        let item = memory_item("Forget projected memory");
        let item_path = item_relative_path(&item);
        write_memory_vault(dir.path(), std::slice::from_ref(&item), &[], &[]).unwrap();
        let user_path = dir.path().join("memory/items/user-note.md");
        fs::write(&user_path, "# User owned\n").unwrap();

        let export = write_memory_vault(dir.path(), &[], &[], &[]).unwrap();

        assert!(!dir.path().join(&item_path).exists());
        assert!(user_path.exists());
        assert!(export.files_removed.contains(&path_to_markdown(&item_path)));
    }
